...
However, as chips have become faster, focusing on wirelength reduction can lead to longer pathlengths, negatively impacting critical path timing.  
...
In Step 2, Parallelization, we identify a set of nets whose topologies can be improved simultaneously.  
This allows us to extract timing-independent nets for parallel processing.  
In Step 3, we apply our refinement algorithm to each net.  
I'll explain this in detail on the next slides.  
Finally, we complete global routing by performing the remaining GR steps for the refined topologies.  
...
In the first step, we identify the critical path based on the previous STA results.  
...
We place a higher score on delay improvements for pins with negative slack.  
To achieve this, we introduced a weighting function, shown in the right figure.  
This function discounts delay improvements for pins with large positive slack.  
As the refinement converges towards maximizing total gain, it reinforces the reduction of wire delay to critical pins.  
A key challenge is estimating wire delay changes accurately in the early stages.  
...
We estimate wire delay changes using differences before and after perturbation.  
For Elmore delay calculations, we use the resistance and capacitance per unit length of Metal 3.  
...
Predicting cell delay is crucial as cell delay impacts all connected pins.  
...
This term prevents excessive increases in wirelength.  
As a result, it helps reduce power consumption and routing congestion.  
...
Importantly, our method works smoothly with existing global routing tools, making it easy to adopt.
Our experiments showed great results. We significantly improved circuit timing without compromising power consumption or wirelength.
In the end, we produced chips with better overall performance. We believe this approach can really boost the efficiency and performance of chip designs.  
