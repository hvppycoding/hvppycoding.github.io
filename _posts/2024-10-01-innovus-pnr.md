...
if { [file exists $outDir/01_Placement.enc ] } {
    ...
    ## If you only need the placement result (-r placeOnly )	
    if {[info exists REVISION] && $REVISION == "placeOnly"} {
        ...
        defOut -floorplan -netlist $outDir/placeOnly.def 
        summaryReport -outfile $outDir/placeOnly.summary.rpt -noHtml
        report_analysis_summary > $outDir/placeOnly.analysis.summary.rpt
        report_resource -verbose
        exit
    }
...
if { [file exists $outDir/03_ccopt.enc ] } {
    ...
    if { [ file exists "config_pre_ccopt.tcl" ] } { source config_pre_ccopt.tcl }
    ccopt_design -outDir log/clock_report 
    if { [ file exists "config_post_ccopt.tcl" ] } { source config_post_ccopt.tcl }
